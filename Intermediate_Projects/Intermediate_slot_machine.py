...
def deposit():
    while True:
        ...
        if amount.isdigit():
            amount = int(amount)  # isdigit only work on string and give the string
            if amount > 0:
                break
            else:
...
def get_number_of_lines():
    while True:
        ...
        if lines.isdigit():
            lines = int(lines)  # isdigit only work on string and give the string
    ...
    while True:
        ...
        if amount.isdigit():
            ...
            if amount > 0:
                ...
                print(f"Amount must be between Rs{MIN_BET} to Rs{MAX_BET}.")
...
def spin(balance):
    ...
    print(
        f"Your are betting Rs {bet} on {lines} lines. Total bet is equal to : {total_bet}"
    )
